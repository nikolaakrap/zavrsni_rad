    ...
    def update_servo(self):
        ...
        try:
            if self.stylus_tracking:
                ...
                r = R.from_quat(quat) * R.from_quat(quat_before).inv()
                angular_velocity = r.as_euler('xyz')
                ...
                linear_difference = math.sqrt(linear_velocity[0]**2+linear_velocity[1]**2+linear_velocity[2]**2)
                angular_difference = np.linalg.norm(angular_velocity)
                self.get_logger().info(f"Distance to kost marker: {linear_difference} meters")
                self.get_logger().info(f"Distance to kost marker: {angular_difference} radians")

                if linear_difference > 0.005 and angular_difference > 0.0009:
                    self.servo.servo(
                        linear=(linear_velocity[0], linear_velocity[1], linear_velocity[2]),
                        angular=(angular_velocity[0], angular_velocity[1], angular_velocity[2]),
                        enable_if_disabled=True
